...
void menuRegras()
{
	...
	regrasJogo.setIcon(header.getSize().x, header.getSize().y, header.getPixelsPtr());

	sf::Texture bgRulesTex;
	bgRulesTex.loadFromFile("assets/menuRegras.png");
	sf::Sprite bgRules(bgRulesTex);
	float escalaX = regrasJogo.getSize().x / static_cast<float>(bgRulesTex.getSize().x);
	float escalaY = regrasJogo.getSize().y / static_cast<float>(bgRulesTex.getSize().y);
...
int main(int argc, char **argv) {
	sf::RenderWindow paginaInicial(sf::VideoMode(1400, 800), "Sponge Bob SquarePants MazeCraze");

	    sf::Image header;
	    header.loadFromFile("assets/logoJogo.png");
	    paginaInicial.setIcon(header.getSize().x, header.getSize().y, header.getPixelsPtr());

	    sf::Texture backgroundTexture;
	    backgroundTexture.loadFromFile("assets/backgroundInicial.png");
	    sf::Sprite background(backgroundTexture);
	    float escalaX = paginaInicial.getSize().x / static_cast<float>(backgroundTexture.getSize().x);
	    float escalaY = paginaInicial.getSize().y / static_cast<float>(backgroundTexture.getSize().y);
